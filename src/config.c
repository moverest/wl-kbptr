...
#define SECTION(name, desc, ...)                                             \
    (struct section_def) {                                                   \
        #name, desc, offsetof(struct config, name), (struct field_def *[]) { \
            __VA_ARGS__, NULL                                                \
        }                                                                    \
    }
...
static struct section_def section_defs[] = {
    ...
    SECTION(
        mode_click, "Configuration for click mode",
        MC_FIELD(button, "left", parse_click, noop, "Mouse button to trigger")
    ),
};
